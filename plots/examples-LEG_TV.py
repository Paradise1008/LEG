...
for i in range(len(img_name)):
    ...
    img = image.load_img('C:/Users/luosh/Desktop/LEG/Image/'+img_name[i]+'.jpg', target_size=(224,224))
    ...
    begin_time = time()
    LEG_small  = LEG_explainer(np.expand_dims(img.copy(), axis = 0), VGG19_MODEL, predict_vgg19,base_size = 28, num_sample = 4000, penalty=None, noise_lvl=0.02)
    LEGTV_small  = LEG_explainer(np.expand_dims(img.copy(), axis = 0), VGG19_MODEL, predict_vgg19,base_size = 28, num_sample = 4000, penalty='TV',lambda_arr = [0.3],noise_lvl=0.02)
    end_time = time()
